...
The upstream development repository compiles all five schemas with ajv's draft
2020-12 standalone generator using `strict: true` and `allErrors: true`. This
runtime package intentionally omits that development-only generator and ships
`renderers/shared/generated-validators.mjs`, so validation needs no npm or
...
Generator drift checks run in the upstream development repository. This runtime package validates inputs with the committed standalone validators.
